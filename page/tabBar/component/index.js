import i18next from "i18next";
const basicContainers = [
  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.BasicView",
      "基础视图"
    ),
    thumb: "/image/icon/view.png",
    nameEn: "View",
    path: "/page/component/view/view",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.ScrollView",
      "滚动视图"
    ),
    thumb: "/image/icon/scroll-view.png",
    nameEn: "ScrollView",
    path: "/page/component/scroll-view/scroll-view",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.SlidingView",
      "滑动视图"
    ),
    thumb: "/image/icon/swiper.png",
    nameEn: "Swiper",
    path: "/page/component/swiper/swiper",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.RemovableView",
      "可移动视图"
    ),
    thumb: "/image/icon/movable-view.png",
    nameEn: "MovableView",
    path: "/page/component/movable-view/movable-view",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.NativeView",
      "原生视图"
    ),
    thumb: "/image/icon/cover-view.png",
    nameEn: "CoverView",
    path: "/page/component/cover-view/cover-view",
  },
];

const basicBasics = [
  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Text", "文字"),
    thumb: "/image/icon/text.png",
    nameEn: "Text",
    path: "/page/component/text/text",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Icon", "图标"),
    thumb: "/image/icon/icon.png",
    nameEn: "Icon",
    path: "/page/component/icon/icon",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.RichText",
      "富文本"
    ),
    thumb: "/image/icon/form.png",
    nameEn: "RichText",
    path: "/page/component/rich-text/rich-text",
  },
];

const basicFeedBacks = [
  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.ProgressBar",
      "进度条"
    ),
    thumb: "/image/icon/progress.png",
    nameEn: "Progress",
    path: "/page/component/progress/progress",
  },
];

const basicForms = [
  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Button", "按钮"),
    thumb: "/image/icon/button.png",
    nameEn: "Button",
    path: "/page/component/button/button",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Form", "表单"),
    thumb: "/image/icon/form.png",
    nameEn: "Form",
    path: "/page/component/form/form",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Tag", "标签"),
    thumb: "/image/icon/label.png",
    nameEn: "Label",
    path: "/page/component/label/label",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.InputBox",
      "输入框"
    ),
    thumb: "/image/icon/input.png",
    nameEn: "Input",
    path: "/page/component/input/input",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.MultiLineInputBox",
      "多行输入框"
    ),
    thumb: "/image/icon/textarea.png",
    nameEn: "Textarea",
    path: "/page/component/textarea/textarea",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.SingleBead",
      "单选框"
    ),
    thumb: "/image/icon/radio.png",
    nameEn: "Radio",
    path: "/page/component/radio/radio",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.CheckBox",
      "复选框"
    ),
    thumb: "/image/icon/checkbox.png",
    nameEn: "Checkbox",
    path: "/page/component/checkbox/checkbox",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Switch", "开关"),
    thumb: "/image/icon/switch.png",
    nameEn: "Switch",
    path: "/page/component/switch/switch",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.SlidingBar",
      "滑动条"
    ),
    thumb: "/image/icon/slider.png",
    nameEn: "Slider",
    path: "/page/component/slider/slider",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.SelectorView",
      "选择器视图"
    ),
    thumb: "/image/icon/picker-view.png",
    nameEn: "PickerView",
    path: "/page/component/picker-view/picker-view",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.Selector",
      "选择器"
    ),
    thumb: "/image/icon/picker.png",
    nameEn: "Picker",
    path: "/page/component/picker/picker",
  },
];

const basicNavigators = [
  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.Navigation",
      "导航"
    ),
    thumb: "/image/icon/navigator.png",
    nameEn: "Navigator",
    path: "/page/component/navigator/navigator",
  },
];

const basicMedias = [
  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Image", "图片"),
    thumb: "/image/icon/image.png",
    nameEn: "Image",
    path: "/page/component/image/image",
  },
];

const basicMaps = [
  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Map", "地图"),
    thumb: "/image/icon/map.png",
    nameEn: "Map",
    path: "/page/component/map/map",
  },
];

const basicCanvas = [
  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Canvas", "画布"),
    thumb: "/image/icon/canvas.png",
    nameEn: "Canvas",
    path: "/page/component/canvas/canvas",
  },
];

const basicOpens = [
  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.EmbeddedWebview",
      "内嵌webview"
    ),
    thumb: "/image/icon/webview.png",
    nameEn: "Webview",
    path: "/page/component/webview/webview",
  },

  // {
  //   name: '关注生活号',
  //   thumb: '/image/icon/lifestyle.png',
  //   nameEn: 'Lifestyle',
  //   path: '/page/component/lifestyle/lifestyle',
  // },
  // {
  //   name: '智能客服',
  //   thumb: '/image/icon/contact-button.png',
  //   nameEn: 'contact-button',
  //   path: '/page/component/contact-button/contact-button',
  // },
  // {
  //   name: '收藏',
  //   thumb: '/image/icon/favorite.png',
  //   nameEn: 'Favorite',
  //   path: '/page/component/favorite/favorite',
  // },
];

let basicComponentList = [
  {
    type: i18next.t(
      "ant-mini-program-demo.tabBar.component.ViewContainer",
      "视图容器"
    ),
    list: basicContainers,
  },

  {
    type: i18next.t(
      "ant-mini-program-demo.tabBar.component.BasicComponents",
      "基础组件"
    ),
    list: basicBasics,
  },

  {
    type: i18next.t("ant-mini-program-demo.tabBar.component.Feedback", "反馈"),
    list: basicFeedBacks,
  },

  {
    type: i18next.t("ant-mini-program-demo.tabBar.component.Form", "表单"),
    list: basicForms,
  },

  {
    type: i18next.t(
      "ant-mini-program-demo.tabBar.component.Navigation",
      "导航"
    ),
    list: basicNavigators,
  },

  {
    type: i18next.t("ant-mini-program-demo.tabBar.component.Media", "媒体"),
    list: basicMedias,
  },

  {
    type: i18next.t("ant-mini-program-demo.tabBar.component.Canvas", "画布"),
    list: basicCanvas,
  },
];

if (my.ap) {
  basicComponentList = basicComponentList.concat([
    {
      type: i18next.t("ant-mini-program-demo.tabBar.component.Map", "地图"),
      list: basicMaps,
    },

    {
      type: i18next.t(
        "ant-mini-program-demo.tabBar.component.OpenComponents",
        "开放组件"
      ),
      list: basicOpens,
    },
  ]);
}

const extContainers = [
  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Container", "容器"),
    thumb: "/image/icon/container.png",
    nameEn: "Container",
    path: "/page/component/container/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.ModuleTitle",
      "模块标题"
    ),
    thumb: "/image/icon/navigator.png",
    nameEn: "Title",
    path: "/page/component/title/index",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.List", "列表"),
    thumb: "/image/icon/form.png",
    nameEn: "List",
    path: "/page/component/list/list",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.ListElements",
      "列表元素"
    ),
    thumb: "/image/icon/list-item.png",
    nameEn: "List-item",
    path: "/page/component/list-item/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.TopTab",
      "顶部选项卡"
    ),
    thumb: "/image/icon/tabs.png",
    nameEn: "Tabs",
    path: "/page/component/tabs/tabs",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.VerticalTab",
      "纵向选项卡"
    ),
    thumb: "/image/icon/vtabs.png",
    nameEn: "Vtabs",
    path: "/page/component/vtabs/vtabs",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Card", "卡片"),
    thumb: "/image/icon/card.png",
    nameEn: "Card",
    path: "/page/component/card/card",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Ticket", "票券"),
    thumb: "/image/icon/coupon.png",
    nameEn: "Coupon",
    path: "/page/component/coupon/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.PalaceLattice",
      "宫格"
    ),
    thumb: "/image/icon/grid.png",
    nameEn: "Grid",
    path: "/page/component/grid/grid",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Step", "步骤条"),
    thumb: "/image/icon/steps.png",
    nameEn: "Steps",
    path: "/page/component/steps/steps",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Footer", "页脚"),
    thumb: "/image/icon/footer.png",
    nameEn: "Footer",
    path: "/page/component/footer/footer",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Protocol", "协议"),
    thumb: "/image/icon/terms.png",
    nameEn: "Terms",
    path: "/page/component/terms/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.FlexLayout",
      "Flex 布局"
    ),
    thumb: "/image/icon/view.png",
    nameEn: "Flex",
    path: "/page/component/flex/flex",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.FoldingPanel",
      "折叠面板"
    ),
    thumb: "/image/icon/collapse.png",
    nameEn: "Collapse",
    path: "/page/component/collapse/index",
  },
];

const pops = [
  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Bubbles", "气泡"),
    thumb: "/image/icon/popover.png",
    nameEn: "Popover",
    path: "/page/component/popover/popover",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Filter", "筛选"),
    thumb: "/image/icon/filter.png",
    nameEn: "Filter",
    path: "/page/component/filter/filter",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.DialogBox",
      "对话框"
    ),
    thumb: "/image/icon/modal.png",
    nameEn: "Modal",
    path: "/page/component/modal/modal",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.PopUpMenu",
      "弹出菜单"
    ),
    thumb: "/image/icon/popup.png",
    nameEn: "Popup",
    path: "/page/component/popup/popup",
  },
];

const extForms = [
  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.TextInput",
      "文本输入"
    ),
    thumb: "/image/icon/input.png",
    nameEn: "Input-item",
    path: "/page/component/input-item/input-item",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.VerificationCodeBox",
      "验证码框"
    ),
    thumb: "/image/icon/verifycode.png",
    nameEn: "Verify-code",
    path: "/page/component/verify-code/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.SelectInput",
      "选择输入"
    ),
    thumb: "/image/icon/picker.png",
    nameEn: "Picker-item",
    path: "/page/component/picker-item/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.LongPasswordBox",
      "长密码框"
    ),
    thumb: "/image/icon/longpassword.png",
    nameEn: "Long-password",
    path: "/page/component/long-password/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.MultiLineInput",
      "多行输入"
    ),
    thumb: "/image/icon/textarea.png",
    nameEn: "Multi-liner",
    path: "/page/component/multi-liner/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.AmountInput",
      "金额输入"
    ),
    thumb: "/image/icon/amount-input.png",
    nameEn: "Amount-input",
    path: "/page/component/amount-input/amount-input",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Button", "按钮"),
    thumb: "/image/icon/button.png",
    nameEn: "Button",
    path: "/page/component/button-ali-ui/index",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Switch", "开关"),
    thumb: "/image/icon/switch.png",
    nameEn: "Am-switch",
    path: "/page/component/switch-ali-ui/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.CheckBox",
      "复选框"
    ),
    thumb: "/image/icon/checkbox-aliui.png",
    nameEn: "Am-checkbox",
    path: "/page/component/am-checkbox/am-checkbox",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.SingleBead",
      "单选框"
    ),
    thumb: "/image/icon/radio-aliui.png",
    nameEn: "Am-radio",
    path: "/page/component/radio-ali-ui/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.SearchBox",
      "搜索框"
    ),
    thumb: "/image/icon/search-bar.png",
    nameEn: "Search-bar",
    path: "/page/component/search-bar/search-bar",
  },
];

const results = [
  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.ExceptionPage",
      "异常页"
    ),
    thumb: "/image/icon/page-result.png",
    nameEn: "Page-result",
    path: "/page/component/page-result/page-result",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.ResultPage",
      "结果页"
    ),
    thumb: "/image/icon/message.png",
    nameEn: "Message",
    path: "/page/component/message/message",
  },
];

const tips = [
  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Guide", "引导"),
    thumb: "/image/icon/tips.png",
    nameEn: "Tips",
    path: "/page/component/tips/tips",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.BulletinBoard",
      "通告栏"
    ),
    thumb: "/image/icon/notice.png",
    nameEn: "Notice",
    path: "/page/component/notice/notice",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Logo", "徽标"),
    thumb: "/image/icon/view.png",
    nameEn: "Badge",
    path: "/page/component/badge/badge",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Tag", "标签"),
    thumb: "/image/icon/tag.png",
    nameEn: "Tag",
    path: "/page/component/tag/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.BackgroundMask",
      "背景蒙层"
    ),
    thumb: "/image/icon/mask.png",
    nameEn: "Mask",
    path: "/page/component/mask/mask",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.MaskGuide",
      "遮罩引导"
    ),
    thumb: "/image/icon/guide.png",
    nameEn: "Guide",
    path: "/page/component/guide/guide",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Avatar", "头像"),
    thumb: "/image/icon/avatar.png",
    nameEn: "Avatar",
    path: "/page/component/avatar/index",
  },
];

const gestures = [
  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.SlidingCell",
      "可滑动单元格"
    ),
    thumb: "/image/icon/swipe-action.png",
    nameEn: "Swipe-action",
    path: "/page/component/swipe-action/swipe-action",
  },
];

const inputs = [];

const others = [
  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.PaginationCharacter",
      "分页符"
    ),
    thumb: "/image/icon/pagination.png",
    nameEn: "Pagination",
    path: "/page/component/pagination/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.LetterSearchTable",
      "字母检索表"
    ),
    thumb: "/image/icon/alphabet.png",
    nameEn: "Alphabet",
    path: "/page/component/alphabet/index",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.WalkingDevice",
      "步进器"
    ),
    thumb: "/image/icon/stepper.png",
    nameEn: "Stepper",
    path: "/page/component/stepper/stepper",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Calendar", "日历"),
    thumb: "/image/icon/calendar.png",
    nameEn: "Calendar",
    path: "/page/component/calendar/calendar",
  },

  {
    name: i18next.t("ant-mini-program-demo.tabBar.component.Icon", "图标"),
    thumb: "/image/icon/icon.png",
    nameEn: "Am-icon",
    path: "/page/component/am-icon/am-icon",
  },

  {
    name: i18next.t(
      "ant-mini-program-demo.tabBar.component.Loading",
      "loading 加载"
    ),
    thumb: "/image/icon/loading.png",
    nameEn: "Loading",
    path: "/page/component/loading/loading",
  },
];

const extComponentList = [
  {
    type: i18next.t(
      "ant-mini-program-demo.tabBar.component.LayoutNavigation",
      "布局导航"
    ),
    list: extContainers,
  },

  {
    type: i18next.t(
      "ant-mini-program-demo.tabBar.component.OperationFloatingLayer",
      "操作浮层"
    ),
    list: pops,
  },

  {
    type: i18next.t("ant-mini-program-demo.tabBar.component.Result", "结果类"),
    list: results,
  },

  {
    type: i18next.t(
      "ant-mini-program-demo.tabBar.component.PromptGuide",
      "提示引导"
    ),
    list: tips,
  },

  {
    type: i18next.t("ant-mini-program-demo.tabBar.component.Form.1", "表单类"),
    list: extForms,
  },

  {
    type: i18next.t("ant-mini-program-demo.tabBar.component.Gesture", "手势类"),
    list: gestures,
  },

  {
    type: i18next.t("ant-mini-program-demo.tabBar.component.Others", "其他"),
    list: others,
  },
];

Page({
  data: {
    top: 0,
    hot: [
      { name: "ScrollView", url: "/page/component/scroll-view/scroll-view" },
      {
        name: i18next.t("ant-mini-program-demo.tabBar.component.Map", "地图"),
        url: "/page/component/map/map",
      },
      { name: "Icon", url: "/page/component/icon/icon" },
      { name: "Card", url: "/page/component/card/card" },
      // { name: '获取授权码', url: '/page/API/get-auth-code/get-auth-code' },
      { name: "Popup", url: "/page/component/popup/popup" },
      {
        name: i18next.t(
          "ant-mini-program-demo.tabBar.component.InitiateAnHttpRequest",
          "发起HTTP请求"
        ),
        url: "/page/API/request/request",
      },
      {
        name: i18next.t(
          "ant-mini-program-demo.tabBar.component.Canvas",
          "画布"
        ),
        url: "/page/component/canvas/canvas",
      },
      {
        name: i18next.t(
          "ant-mini-program-demo.tabBar.component.Navigation",
          "导航"
        ),
        url: "/page/API/navigator/navigator",
      },
    ],

    tabs: [
      i18next.t(
        "ant-mini-program-demo.tabBar.component.BasicComponents",
        "基础组件"
      ),
      i18next.t(
        "ant-mini-program-demo.tabBar.component.ExtendedComponents",
        "扩展组件"
      ),
    ],
    activeTab: 0,
    basicComponentList,
    extComponentList,
    titleOpacity: 1,
    shadow: false,
    I18N_T: (i18next.getDataByLanguage(i18next.language) || {}).translation,
  },

  onPageScroll(e) {
    const { scrollTop } = e;
    let titleOpacity = 1 - scrollTop * 0.02;
    let shadow = false;

    if (titleOpacity < 0) {
      titleOpacity = 0;
    }

    if (titleOpacity > 1) {
      titleOpacity = 1;
    }

    if (scrollTop > 80) {
      my.setNavigationBar({
        title: i18next.t(
          "ant-mini-program-demo.tabBar.component.OfficialExamplesOfMpaasMini",
          "mPaaS小程序官方示例"
        ),
      });
    } else {
      my.setNavigationBar({
        title: " ",
      });
    }

    if (scrollTop > 320) {
      shadow = true;
    } else {
      shadow = false;
    }

    this.setData({
      shadow,
      titleOpacity,
    });
  },
  onSearchBarTap() {
    my.navigateTo({
      url: "/page/common/search/search",
    });
  },
  onTabBarTap(e) {
    const { index } = e.target.dataset;
    this.setData({
      activeTab: index,
    });
  },
  onLoad() {
    my.getSystemInfo({
      success: (res) => {
        if (res.statusBarHeight && res.titleBarHeight) {
          this.setData({
            top: res.statusBarHeight + res.titleBarHeight,
          });
        }
      },
    });
  },
  goDevCenter() {
    my.navigateToMiniProgram({
      appId: "2018082061148052",
      path: "pages/discover/discover",
      extraData: {
        from: "miniDemo",
      },
    });
  },
  openPage(e) {
    my.navigateTo({
      url: e.target.dataset.url,
    });
  },
});
