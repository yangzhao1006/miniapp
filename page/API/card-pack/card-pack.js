    ...
    my.openCardList({
      success: (res) => {
        my.showToast({
          content:
            i18next.t(
              "ant-mini-program-demo.API.card-pack.card-pack.SuccessfulCall",
              "调用成功："
            ) + JSON.stringify(res),
        });
      },
      fail: (error) => {
        my.alert({
          content:
            i18next.t(
              "ant-mini-program-demo.API.card-pack.card-pack.FailedToCall",
              "调用失败："
      ...
      complete: (res) => {
        my.alert({
          content: i18next.t(
            "ant-mini-program-demo.API.card-pack.card-pack.CompleteTheCallback",
            "complete回调！"
          ),
        });
      },
