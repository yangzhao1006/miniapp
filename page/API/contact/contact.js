...
Page({
  data: {
    ...
    nickName: i18next.t(
      "ant-mini-program-demo.API.contact.contact.JulyFire",
      "七月流火"
    ),
    lastName: "Last",
    middleName: "Middle",
    firstName: "First",
    remark: i18next.t(
      "ant-mini-program-demo.API.contact.contact.HereIsTheRemarks",
      "这里是备注"
    ),
  ...
  chooseContact() {
    my.chooseContact({
      ...
      multiChooseMaxTips: i18next.t(
        "ant-mini-program-demo.API.contact.contact.TheMaximumNumberOfPeople",
        "超过选择的最大人数了"
      ),
      success: (res) => {
        my.alert({
  ...
  addPhoneContact() {
    if (my.canIUse("addPhoneContact")) {
      ...
      my.alert({
        title: i18next.t(
          "ant-mini-program-demo.API.contact.contact.TheClientVersionIsToo",
          "客户端版本过低"
        ),
        content: i18next.t(
          "ant-mini-program-demo.API.contact.contact.MyAddphonecontactMustBeOr",
          "my.addPhoneContact() 需要 10.1.32 及以上版本"
        ),
      });
    }
  },
